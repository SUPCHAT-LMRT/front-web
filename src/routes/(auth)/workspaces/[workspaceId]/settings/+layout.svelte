<script lang="ts">
    ...
    let activeTab: Tab = { displayName: "Général", label: "general" };
            ...
            <div>
                <Tabs.Root value={activeTab?.label}>
                    <Tabs.List class="ml-auto dark:bg-gray-800">
                        <Tabs.Trigger value="account" class="text-gray-600 dark:text-gray-200" onclick={() => handleTabChange({ displayName: "Général", label: "general" }, `/workspaces/${currentWorkspaceId}/settings/general`)}>
                            Général
                        </Tabs.Trigger>
                        <Tabs.Trigger value="confidentiality" class="text-gray-600 dark:text-gray-200" onclick={() => handleTabChange({ displayName: "Confidentialité", label: "confidentiality" }, `/workspaces/${currentWorkspaceId}/settings/confidentiality`)}>
                            Confidentialité
                        </Tabs.Trigger>
                        <Tabs.Trigger value="roles" class="text-gray-600 dark:text-gray-200" onclick={() => handleTabChange({ displayName: "Rôles", label: "roles" }, `/workspaces/${currentWorkspaceId}/settings/roles`)}>
                            Rôles
                        </Tabs.Trigger>
                    </Tabs.List>
                </Tabs.Root>
