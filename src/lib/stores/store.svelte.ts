...
export abstract class Store<T> {
    ...
    private readonly store: Writable<StoreResult<T>> = writable<StoreResult<T>>(this.state);

    public subscribe(run: (value: StoreResult<T>) => void, invalidate?: (value?: StoreResult<T>) => void) {
    ...
    public reset() {
        this.changeStateAndData(this.getDefaultState().state, this.getDefaultState().data);
    }
    ...
    protected changeStateAndError(state: StoreResultState, error: Error, displayCode: number) {
        this.state.state = state;
